...
order = 14
ncoils = 5
length_target_initial = 11
length_target_final = 13
length_weight = 1e-1
cc_weight = 1e-1
max_curvature_threshold = 3.5
...
s = SurfaceRZFourier.from_vmec_input('input.W7-X_standard_configuration', range="half period", nphi=nphi, ntheta=ntheta)
# base_curves = create_equally_spaced_curves(ncoils, s.nfp, stellsym=True, R0=R0, R1=R1, order=order)
vmec = Vmec('wout_W7-X_standard_configuration.nc', verbose=False)
...
curves_to_vtk(curves, os.path.join(parent_path,"curves_init"))
...
s.to_vtk(os.path.join(parent_path, "surf_init"), extra_data=pointData)
...
bs.set_points(s.gamma().reshape((-1, 3)))
curves_to_vtk(curves, os.path.join(parent_path,"curves_opt"))
Bbs = bs.B().reshape((nphi, ntheta, 3))
BdotN_surf = np.sum(Bbs * s.unitnormal(), axis=2) / np.linalg.norm(Bbs, axis=2)
Bmod = bs.AbsB().reshape((nphi,ntheta,1))
pointData = {"B.n/B": BdotN_surf[:, :, None], "B": Bmod}
s.to_vtk(os.path.join(parent_path, "surf_opt"), extra_data=pointData)
bs.save(os.path.join(parent_path, "biot_savart_opt.json"))
...
pointData = {"B.n/B": BdotN_surf[:, :, None], "B": Bmod}
surf_big.to_vtk(os.path.join(parent_path, "surf_opt_big"), extra_data=pointData)